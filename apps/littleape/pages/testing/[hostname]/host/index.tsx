...
import {
  ...
  HostData,
  RoomData,
  convertRoomDataToFormData,
} from "lib/pocketBase/helperAPI";
    ...
    if (roomsList.code != undefined && roomsList.code == 404) {
      ...
      if (room != undefined && room != null) {
        ...
        if (room.thumbnail != "" && meetingStore.selectedImage == null && resetThumbnail == false) {
          meetingStore.thumbnailUrl = pbApi.thumbnailUrl(
            room.collectionId,
            room.id,
            room.thumbnail
          );
  ...
  const ShowLinksComponent = () => {
    return (
      <div className="p-5 flex flex-col gap-5 pb-6">
        <span className="text-bold-12 text-gray-2">
          Copy and use host’s link for yourself, and audience link for sending to others:
        </span>
      ...
      <div className="w-full justify-center items-center px-4 min-h-full flex flex-col min-h-screen">
        <div className="w-full max-w-[632px] mx-auto mt-10 border rounded-md border-gray-300">
          <form className="flex flex-col w-full">
            ...
            <div className="p-5 flex flex-col gap-5 sm:px-16">
              <div className="flex flex-col gap-3">
                <div className="mb-4">
                  <span className="block text-semi-bold-32 text-secondary-a-1">
                    Start A Live Conference
                  </span>
                  <span className="block text-regular-12 text-gray-2">
                    Please enter your display name and conference info:
                  </span>
                </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="my-0 flex items-center justify-between relative h-8">
                  ...
                  <div
                    className="text-bold-12 text-gray-1 cursor-pointer float-right cursor-pointer"
                    onClick={() => {
                  ...
                  >
                    {meetingStore.selectedCssFile != null
                      ? meetingStore.selectedCssFile.name
                      : "Default"}{" "}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between relative h-8">
                  ...
                  <Image
                    ...
                    className="rounded-md float-right cursor-pointer border border-black border-1"
                    src={
                      meetingStore.thumbnailUrl
                        ? meetingStore.thumbnailUrl
                        : meetingStore.selectedImage
                        ? meetingStore.selectedImage
                        : LogoIcon
                    }
                    onClick={() => {
          ...
          <ResponsiveModal
            ...
            onClose={setShowProfileModal.bind(null, false)}
            maxWidth="550px"
          >
            ...
            <div className="flex items-center gap-3 px-4 max-w-full">
              {userProfile.image ? (
                <img
                  src={userProfile.image}
                  alt="User Avatar"
            ...
            <div
              className="flex items-center space-x-3 mx-4 mb-4 cursor-pointer"
              onClick={() => {
          ...
          <ResponsiveModal
            open={showEventScheduleModal}
            onClose={setShowEventScheduleModal.bind(null, false)}
            modalProps={{ disableEnforceFocus: true }}
          >
            <span className="text-bold-12 text-black block text-center pt-5">
              Schedule The Live Room
            </span>
            <hr className="mt-4 mb-1 border-white md:border-gray-0" />
            <div className="p-5 flex flex-col gap-5 pb-6">
              <span className="text-bold-12 text-gray-2">
                Please enter your desirable date and time for starting the event:
              </span>
          ...
          <ResponsiveModal
            open={showEventLinksModal}
            onClose={setShowEventLinksModal.bind(null, false)}
          >
            <span className="text-bold-12 text-black block text-center pt-5">Room Links</span>
            <hr className="mt-4 mb-1 border-white md:border-gray-0" />
            <div className="p-5 pb-0 flex flex-col gap-5">
              <span className="text-bold-14 text-black">
                The GreatApe conversation was shared with your followers on your {displayName}{" "}
                account. You can start your live show at {dateTimeFromUnix}
              </span>
            </div>
