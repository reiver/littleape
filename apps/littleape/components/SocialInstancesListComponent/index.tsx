...
const getInstanceList = (title) => {
  if (title == SocialPlatform.MASTODON) {
    return [
      new SocialPlatformInsatnce("Mastodon.Social", "5K"),
      new SocialPlatformInsatnce("Mas.to", "6K"),
      new SocialPlatformInsatnce("Social.Vivaldi.net", "13K"),
      new SocialPlatformInsatnce("Mstdn.ca", "10K"),
    ];
  } else if (title == SocialPlatform.PIXELFED) {
    return [
      new SocialPlatformInsatnce("Pixelfed.Social", "5K"),
      new SocialPlatformInsatnce("Pixelfed.de", "6K"),
      new SocialPlatformInsatnce("Pixelfed.uno", "13K"),
      new SocialPlatformInsatnce("Gram.Social", "10K"),
    ];
  } else if (title == SocialPlatform.MISSKEY) {
    return [
      new SocialPlatformInsatnce("Misskey.io", "5K"),
      new SocialPlatformInsatnce("Misskey.Social", "1.1K"),
      new SocialPlatformInsatnce("Mk.Godspeed.moe", "61"),
      new SocialPlatformInsatnce("Misskey.id", "1.7K"),
    ];
  } else if (title == SocialPlatform.PEERTUBE) {
    return [
      new SocialPlatformInsatnce("v.basspistol.org", "5K"),
      new SocialPlatformInsatnce("skeptikon.fr", "6K"),
      new SocialPlatformInsatnce("Fair.Tube", "13K"),
      new SocialPlatformInsatnce("Clip.Place", "10K"),
    ];
  }
  return [];
};
...
export const SocialInstancesListComponent = ({
  logo,
  title,
  goBack,
}: SocialInstancesListComponentProps) => {
  ...
  const NewServerModal: FC<NewServerModalProps> = ({ onOk, onCanel }) => {
    ...
    return (
      <div className="absolute top-0 left-0 w-full h-full">
        ...
        <div className="absolute -translate-y-full z-20 top-full left-0 right-0 sm:right-unset sm:top-1/2 sm:left-1/2 transform sm:-translate-x-1/2 sm:-translate-y-1/2 bg-white text-gray-2 sm:rounded-lg rounded-t-lg w-full sm:max-w-[30%] sm:border border-gray-0">
          ...
          <span className="text-bold-12 text-gray-2 mx-4 items-center">
            Please enter the instance server URL:
          </span>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 mt-8 mx-4 mb-4">
            ...
            <button
              className={`h-12 w-full sm:w-1/2 border ${
                newUrl == "" ? "bg-gray-1 border-gray-1" : "bg-secondary-1-a border-black"
              } text-white-f text-[14px] font-bold rounded-md flex items-center justify-center gap-2 py-2`}
              onClick={() => onOk(newUrl)}
    ...
    <>
      <div>
        ...
        <div className="mt-2 text-gray-2 text-regular-14">
          Click next to join <span className="text-bold-14">“{baseLink}”</span>. Or if you want to
          find a community that matches your interests better, or is local to where you live, choose
          ...
          {instancesList.length > 0 &&
            instancesList.map((instance, index) => (
              <div key={index} className="w-full">
                <div
                  className={`relative flex items-center sm:px-6 px-4 border-2 mt-4 ${
                    selectedIndex === index ? "border-secondary-1-a" : "border-gray-0"
                  } mb-3 rounded-lg`}
                  onClick={() => {
                    handleClick(index);
                  }}
                >
                  {selectedIndex === index && (
                    <div className="absolute -top-3 -right-[-18px] rounded-full border-2 border-black bg-primary p-1 items-center">
                  ...
                  <div className="flex-grow sm:py-4 py-2 rounded-md items-center justify-between cursor-pointer">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="flex items-center">
                        <LanguageIcon />
                        <div className="ml-1 text-regular-12 text-gray-1 items-center">EN</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div
          className="flex items-center justify-center text-bold-14 text-secondary-1-a mt-4 cursor-pointer"
      ...
      {showPeerTubeLoginForm && (
        <PeerTubeLoginForm
          instance={baseLink}
          isOpen={isPeerTubeLoginFormOpen}
          onClose={() => {
            onPeerTubeLoginFormClose();
          }}
        />
      )}
    </>
...
const PeerTubeLoginForm: FC<PeerTubeLoginFormProps> = ({ isOpen, onClose, instance, ...props }) => {
  ...
  const handleLogin = async () => {
    ...
    if (!instance || !/^https?:\/\/[a-zA-Z0-9.-]+$/.test(instance)) {
      handleRedirect(
        `/auth/login?peertubeerror=${encodeURIComponent("Invalid or missing instance URL")}`
      );
    }
    ...
    try {
      ...
      if (!registerRes.ok) {
        handleRedirect(
          `/auth/login?peertubeerror=${encodeURIComponent("App registration failed")}`
        );
      }
      ...
      if (accessToken.includes("error")) {
        handleRedirect(
          `/auth/login?peertubeerror=${encodeURIComponent("Failed to get access token")}`
        );
      }
      ...
      if (isPeerTubeUser(account) === true) {
        ...
        handleRedirect(
          `/auth/login?peertubeerror=${encodeURIComponent("Failed to get user account")}`
        );
      }
    } catch (err) {
      handleRedirect(
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full">
      <div className="z-50 absolute w-full h-full bg-black bg-opacity-60" />
      <div className="absolute -translate-y-full z-20 top-full left-0 right-0 sm:right-unset sm:top-1/2 sm:left-1/2 transform sm:-translate-x-1/2 sm:-translate-y-1/2 bg-white text-black-0 sm:rounded-lg rounded-t-lg w-full sm:max-w-[28%] sm:border border-gray-0">
        <div className="flex justify-center items-center p-5 relative">
        ...
        <div className="mx-4 my-4">
          ...
          <div className="flex items-center my-4 justify-center">
            ...
            <span
              className="text-bold-14 text-[#F1680D] cursor-pointer"
              onClick={goToPeerTubeForRegistration}
            >
              {"Register!"}
            </span>
          </div>
