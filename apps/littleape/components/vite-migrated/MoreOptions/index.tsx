...
import { sparkRtcSignal, updateUser } from "components/Meeting";
  ...
  return (
    ...
    >
      <div className="flex flex-col pt-2 gap-2 max-h-full">
        <div className="px-2 flex w-full gap-2 min-h-[36px] min-w-[36px]">
          <span className="text-bold-18 text-gray-2 dark:text-white-f-9">
            {"Welcome to the Fediverse!"}
          </span>
        </div>

        <div className="px-2 flex flex-col justify-center w-full gap-2 min-h-[36px] min-w-[36px]">
          <span className="text-semi-bold-16 text-gray-2 dark:text-white-f-9">{"About"}</span>
          <span className="text-medium-12 text-gray-2 dark:text-white-f-9">
            {"Welcome to the Fediverse!"}
          </span>
        </div>
        ...
        <div className="flex flex-col gap-2 w-full mt-4 pt-2 overflow-auto cursor-pointer">
          ...
          <div className="flex items-center truncate cursor-pointer" onClick={handleRecording}>
            <div className="rounded-full w-9 h-9 flex justify-center items-center">
              {isRecordingStarted ? <Icon icon={RecordStop} /> : <Icon icon={RecordStart} />}
            </div>
...
const Icon = ({ icon: IconComponent }) => {
  return <IconComponent width="24px" height="24px" className="greatape-attendees-item" />;
};
