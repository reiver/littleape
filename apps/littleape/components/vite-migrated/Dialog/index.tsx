        ...
        <form>
          <div className="sm:pb-4 pb-2">
            {devices.map((device, index) => (
              <div className="sm:py-4 py-2 rounded-md mx-2 flex items-center cursor-pointer" onClick={() => handleDeviceClick(index)}>
                <Icon
                  icon={
      ...
      >
        ...
        {showButtons && (
          <div className="flex justify-end gap-2 p-5 pt-0">
            ...
            <Button size="lg" variant={okButtonVariant} className="w-full flex-grow-1" onClick={onOk}>
              {okText}
            </Button>
          </div>
        )}
...
export const InviteDialog = ({
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full">
      <div className="z-10 absolute w-full h-full bg-black bg-opacity-60" onClick={onClose} />
      <div
        className={clsx(
          className,
          'absolute -translate-y-full z-20 top-full left-0 right-0 sm:right-unset sm:top-1/2 sm:left-1/2 transform sm:-translate-x-1/2 sm:-translate-y-1/2 dark:bg-gray-3 dark:text-gray-0 bg-white text-gray-2 sm:rounded-lg rounded-t-lg w-full sm:max-w-[400px] sm:border dark:border-gray-1 border-gray-0'
