...
import { onInviteToStage, onUserRaisedHand, sparkRtcSignal } from "components/Meeting";
...
const Participant = ({ participant }) => {
  ...
  function checkUserCount() {
    ...
    if (
      sparkRtcSignal.value.sentRequests.length > 0 &&
      sparkRtcSignal.value.acceptedRequests.length > 0 &&
      sparkRtcSignal.value.raiseHands.length >= sparkRtcSignal.value.maxRaisedHands
    ) {
      makeDialog("info", {
    ...
    >
      <div className="flex gap-2 items-center truncate">
        {participant.avatar ? (
          ...
          <div className="dark:bg-gray-300 min-w-[36px] min-h-[36px] dark:bg-opacity-30 bg-opacity-30 bg-gray-400 rounded-full w-9 h-9 flex justify-center items-center">
            <Icon
              ...
              width="24px"
              height="24px"
              className="greatape-attendees-item"
            />
          </div>
        )}
        ...
        <div className="flex flex-col justify-center truncate">
          <span className="text-gray-1 dark:text-gray-0 truncate">
            <span className="text-bold-12 text-gray-3 dark:text-white-f-9 greatape-attendees-item">
              {participant.name}
            </span>{" "}
          </span>
          ...
          {participant.userId == snap.currentUser.userId && participant.isHost ? (
            ...
            participant.isRecordingTheMeeting == true ? (
              <span className="text-gray-1 dark:text-gray-0 text-regular-12 greatape-attendees-item-role">
                Host (You are recording)
              </span>
            ) : (
      ...
      <div className="flex gap-1 dark:text-gray-0 text-gray-1">
        {participant.isRecordingTheMeeting && (
          <Icon
            className={`${
              snap.currentUser.isHost && snap.currentUser.userId != participant.userId
                ? "group-hover:hidden"
                : ""
            } greatape-attendees-item`}
            icon={<RecordingAttendeesList />}
            width="25"
            height="25px"
          />
        )}
        {!raisedHand &&
          ...
          !isMobile && (
            <Icon
              className="hidden group-hover:block greatape-attendees-item"
              icon={<Check />}
              width="25"
              height="25px"
            />
