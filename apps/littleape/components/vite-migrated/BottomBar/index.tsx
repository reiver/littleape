...
export const BottomBar = () => {
  ...
  return (
    <Container className={clsx("transition-all mb-2", {})}>
      <div
        className="w-full grid grid-cols-12 dark:bg-secondary-1-a dark:text-gray-0 text-gray-2"
        id="bottom-bar"
      >
        <div className="col-span-3 sm:block hidden">
          <div className="h-full flex items-center">
            <LinkCopyComponent link={audienceUrl} className="max-w-[300px]" />
          </div>
        </div>
        <div className="col-span-12 sm:col-span-6 flex items-center justify-center">
          {state.broadcastIsInTheMeeting && <Controllers />}
        </div>
        {state.broadcastIsInTheMeeting ? (
          <div className="col-span-3 text-right sm:block hidden">
                ...
                <Tooltip label={state.isAttendeesOpen ? "Hide Attendees" : "Show Attendees"}>
                  ...
                  >
                    <div className="relative">
                      ...
                      {state.attendeesBadge && (
                        <span className="absolute top-0 -right-1 w-2 h-2 rounded-full bg-red-distructive"></span>
                      )}
                    </div>
  ...
  return (
    <BottomSheet open={state.isMoreOptionsOpen} onClose={toggleMoreOptions} title="More">
      <div className="w-full h-full flex gap-3 py-6 flex-col pb-0">
        ...
        <LinkCopyComponent link={audienceUrl} className="max-w-full" />
        <Tooltip label={state.isAttendeesOpen ? "Hide Attendees" : "Show Attendees"}>
          <div
