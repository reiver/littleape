  ...
  return (
    <div className="flex gap-5 py-2">
      ...
      <Tooltip label={isMeetingMuted ? "Listen" : "Deafen"}>
        <IconButton
          variant={isMeetingMuted ? "danger" : undefined}
      ...
      {isStreamming && isHost && (
        <Tooltip
          key={sharingScreenStreamId ? "ShareOff" : "Share"}
          label={!sharingScreenStreamId ? "Share Screen" : "Stop Sharing Screen"}
        >
          <IconButton
      ...
      {((!isStreamming && !disableRaiseHandFeat) || (isStreamming && !isHost)) && (
        <Tooltip
          key={isStreamming ? "Leave the stage" : ableToRaiseHand ? "Raise Hand" : "Put Hand Down"}
          label={
            isStreamming ? "Leave the stage" : ableToRaiseHand ? "Raise Hand" : "Put Hand Down"
          }
        >
          <IconButton
            key={isStreamming ? "hand" : "lower-hand"}
            onClick={onRaiseHand}
            variant={isStreamming || !ableToRaiseHand ? "danger" : undefined}
          >
      ...
      {hasMic && isStreamming && (
        <Tooltip
          ...
          label={!isMicrophoneOn ? "Turn Microphone On" : "Turn Microphone Off"}
        >
      ...
      <Tooltip label={snap.isMoreOptionsOpen ? "Hide Menu" : "Show Menu"}>
        <IconButton
          variant={snap.isMoreOptionsOpen ? "danger" : undefined}
          onClick={toggleMoreOptions}
          className="flex relative"
        >
          <Icon icon={<KebabMenuVertical />} />
          {snap.attendeesBadge && (
            <span className="absolute z-10 top-[0px] right-[0px] w-[10px] h-[10px] rounded-full bg-red-distructive border dark:border-secondary-1-a border-white-f-9"></span>
          )}
        </IconButton>
      </Tooltip>
    </div>
...
export const MoreControllers = () => {
  ...
  const { isHost, sharingScreenStreamId, isStreamming, isMeetingMuted, isRecordingStarted } =
    snap.currentUser;
  const toggleMuteMeeting = () => {
    updateUser({
      isMeetingMuted: !isMeetingMuted,
  ...
  const showNotAbleToStartRecordingDialog = () => {
    makeDialog(
      DialogTypes.RECORDING_NOT_STARTED,
      {
  ...
  return (
    <div className="flex gap-5 py-5 justify-center">
      ...
      <Tooltip label={isRecordingStarted ? "Stop Recording" : "Start Recording"}>
        <IconButton
          variant={isRecordingStarted ? "danger" : undefined}
          className="sm:flex"
