...
export const hasShareScreenStream = computed(
  () => !!Object.values(streamers.value).find((s) => s.isShareScreen)
);
export const hasHostStream = computed(
  () =>
    !!Object.values(streamers.value).find((s) => s.isHost && !s.isShareScreen)
);
...
export const Stage = ({ customStyles }) => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (hostVideoElement) {
        ...
        const positionValue = parseInt(
          computedStyles.getPropertyValue("--position"),
          10
        );

        //get Host Stream
        const hostStream = () => {
          const hostStreamer = Object.values(streamers.value).find(
      ...
      const audienceVideoElement = document.querySelector(
  ...
  return (
    <div
      ...
      style={{ width: `calc(100% - ${attendeesWidth.value}px)` }}
    >
      {broadcastIsInTheMeeting.value ? (
        <div
          class={clsx("relative h-full justify-end", {
            flex: !customStyles,
          })}
        >
          <div
            class={clsx(
              "flex justify-start sm:justify-center items-center h-full transition-all",
              {
            ...
            {Object.values(streamers.value)
              ...
              .map((attendee, i) => {
                ...
                return (
                  <div
                    id={`video_${
                      attendee.isShareScreen ? "sc" : attendee.name
                    }`}
                    key={i}
                    style={clsx(
                      customStyles
      ...
      ) : meetingIsNotStarted.value &&
        meetingStartRemainingTime.value !== "" ? (
        <div>
          <img src={GreatApeImageBeforeMeetingStarted} className="mx-auto" />
          <span class="inline-block w-full text-center text-bold-18">
            The live conversation has not started yet.
            <br />
            Please stand by, and thank you for your patience.
          </span>
          <span class="inline-block w-full text-center text-bold-14 mt-3">
            {meetingStartRemainingTime.value} to go
          </span>
        </div>
      ) : meetingIsEnded.value == true ? (
        <div>
          <img src={GreatApeImageAfterMeetingEnded} className="mx-auto" />
          <span class="inline-block w-full text-center text-bold-14">
...
export const Video = memo(
  ({
    ...
    const [isHoveredOnFullScreenIcon, setHoveredOnFullScreenIcon] =
      useState(false);
    ...
    useEffect(() => {
      if (
        ...
        sparkRTC.value.localStream.id === stream.id
      ) {
      ...
      <div onClick={handleOnClick} className="w-full h-full rounded-lg">
        ...
        <div class="absolute top-0 left-0 flex justify-between w-full px-2 gap-2">
          <div
            ...
            class="flex truncate justify-center items-center greatape-video-name-background"
          >
            <div
              id={`video_name_${isShareScreen ? "sc" : name}`}
              class="px-4 py-1 bg-black bg-opacity-50 text-white rounded-full text-medium-12 truncate greatape-video-name "
            >
              {name}{" "}
              {isHostStream && isShareScreen
                ? "(Shared Screen)"
                : isHostStream
                ? " (Host)"
                : ""}
            ...
            >
              ...
              >
                ...
                >
                  <Icon
                    key={
                      stream && fullScreenedStream.value === stream.id
                        ? ScreenNormal
                        : ScreenFull
                    }
                    icon={
